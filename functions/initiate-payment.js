// Working Netlify function to initiate payment - KPLC Refund Registration
const axios = require('axios');

// PayHero API credentials (working from KPLC project)
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const { phoneNumber, amount = 99, description = 'KPLC Refund Registration Fee' } = requestBody;
    ...
    const externalReference = `KPLC-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    
    // Define the callback URL - use Netlify function URL
    const callbackUrl = `${process.env.URL || process.env.DEPLOY_PRIME_URL || 'https://completeregistration.netlify.app'}/.netlify/functions/payment-callback`;
